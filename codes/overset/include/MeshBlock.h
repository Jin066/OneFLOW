...
{
 private:
  int nnodes;  //** < number of grid nodes */ 网格块节点数
  int ncells;  //** < total number of cells */细胞总数
  int ntypes;  //** < number of different types of cells */单元类型的个数
  int *nv;     //** < number of vertices for each types of cell */每种类型单元的顶点数
  int *nc;     //**  < number of each of different kinds of cells (tets, prism, pyramids, hex etc) */各不同类型单元的数量（三棱镜、金字塔、六角等）
  int nobc;    //** < number of overset boundary nodes */重叠边界节点数
  int nwbc;    //** < number of wall boundary nodes */壁面边界节点数
  //
  double *x;        //** < grid nodes x[3*nnodes] */网格节点x[3 * 节点]
  int *iblank_cell; //** < iblank value at each grid cell */每个网格单元的Iblan k值
  //
  int **vconn;        //** < connectivity of each kind of cell */每种单元的连通性
  int *wbcnode;     //** < wall boundary node indices */壁面边界节点索引
  int *obcnode;     //** < overset boundary node indices */重叠边界节点索引
  uint64_t *cellGID;     //**< Global ID of the cell */单元的全局id
  uint64_t *nodeGID;     //**< Global ID for the nodes */节点的全局id
  //
  double *nodeRes;  //** < node resolution  */ 节点分辨率
  double *userSpecifiedNodeRes;
  double *userSpecifiedCellRes;
  double *elementBbox;   //** < bounding box of the elements */元素的包围框
  int *elementList;      //** < list of elements in */项目清单
  //
  // Alternating digital tree library
  //
  ADT *adt;   /** < Digital tree for searching this block */
  //
  DONORLIST **donorList;      //**< list of donors for the nodes of this mesh */此网格节点的捐赠者列表
  //
  int ninterp;              //**< number of interpolations to be performed */要进行的内插次数
  int interpListSize;
  INTERPLIST *interpList;   //**< list of donor nodes in my grid, with fractions and information of
                                 // who they donate to */我的网格中的捐赠节点列表，包含分数和他们捐赠给谁的信息
  int *interp2donor;

  INTEGERLIST *cancelList;   //** receptors that need to be cancelled because of */需要取消的受体
  int ncancel;              //** conflicts with the state of their donors */与贡献单元状况的冲突
  ...
  int nreceptorCells;      //** number of receptor cells */受体细胞数
  int *ctag;               //** index of receptor cells */受体细胞索引
  int *pointsPerCell;      //** number of receptor points per cell */每个细胞的受体点数
  int maxPointsPerCell;     //** max of pointsPerCell vector */每细胞载体的最大点数
  double *rxyz;            //**  point coordinates */点坐标
  int ipoint; 
  int *picked;             //** < flag specifying if a node has been selected for high-order interpolation */标志，指定是否已选择节点进行高阶插值
  ...
  int *mapmask;    // mask
  int *icft; 	   // frequency table for nodal containment节点控制的频率表
  int mapdims[3];  // dimensions of the map地图的尺寸
  double mapdx[3]; // sides of the map地图的边
 public :
  int *iblank;      //** < iblank value for each grid node */每个网格节点的Iblan k值
  int *iblank_reduced;
  ...
  int nfringe;
  int mexclude;
  int meshtag;    //** < tag of the mesh that this block belongs to */此块所属网格的标记
  int check_uniform_hex_flag;
  double resolutionScale;
  //
  // oriented bounding box of this partition此分区的定向包围框
  // 
  OBB *obb;
  OBB *obh;
  //
  int nsearch;        //** < number of query points to search in this block */要在此块中搜索的查询点数
  int *isearch;       //** < index of query points in the remote process */远程进程中查询点的索引
  int *tagsearch;       //** < index of query points in the remote process */远程进程中查询点的索引
  double *res_search;   //** < resolution of search points */搜索点的分辨率
  int *xtag;            //** < hash to determine if there are duplicates */Hash表以确定是否存在重复
  double *xsearch;    //** < coordinates of the query points */查询点的坐标
  double *rst;            //**  natrural coordinates */自然坐标系
  int *donorId;       //** < donor indices for those found */发现的捐赠者索引
  std::vector<uint64_t> gid_search;   //**< Global node ID for the query points */查询点的全局节点id
  int donorCount;
  int myid;
  double *cellRes;     //** < resolution for each cell */每个单元的分辨率
  int ntotalPoints;        //**  total number of extra points to interpolate */要插值的额外点数总数
  int ihigh;
  int ninterp2;            //** < number of interpolants for high-order points */高阶点的插值数
  int interp2ListSize;
  INTERPLIST *interpList2;    //** < list for high-interpolation points */高插点列表
  int ninterpCart;
  ...
  /**
   * Get donor packet for multi-block/partition setups获取多块/分区设置的施主包
   *
   */
  ...
  /** Reset interpolation list data structure重置插值列表数据结构
   *
   *  Reset the data structures in situations where the performConnectivity在执行连接性的情况下重置数据结构
   *  method is invoked at every timestep when meshes undergo relative motion.当网格经历相对运动时，每个时间步长都会调用方法
   */
