...
class tioga
{
 private :
  ...
  //! Mesh blocks in this processor 此处理器中的网格块
  std::vector<std::unique_ptr<MeshBlock> > mblocks;
  //! Solver assigned mesh tags for the mesh blocks求解器为网格块分配网格标记
  std::vector<int> mtags;
  std::vector<int> mytag;
  //! Mesh tag to local block index lookup mapping网格标记到本地块索引查找映射
  std::map<int, int> tag_iblk_map;

  //! Intersect block unique ID to index lookup mapping用于索引查找映射的相交块唯一id
  std::map<int, int> intBoxMap;

  //! Parallel comms to obblist indicies  obb的并行通信
  std::vector<int> ibsPerProc;
  std::vector<std::vector<int>> ibProcMap;
  //! q-variables registered  已登记的变量
  double **qblock;  //网格块登记的变量
 ...
 public:
  ...
  //** basic destructor */ 基本析构函数
  ~tioga(); 
  
  //** set communicator */ 设置通讯器
  void setCommunicator(MPI_Comm communicator,int id_proc,int nprocs);

  //** registerGrid data */登记网格数据
  ...
  void set_cell_iblank(int *iblank_cell) //设置单元标记
  {
   auto& mb = mblocks[0];
   mb->set_cell_iblank(iblank_cell);
  }

  void set_uniform_hex_flag(int btag, int flag) //设置统一的十六进制标识
  {
      auto idxit = tag_iblk_map.find(btag);
      int iblk = idxit->second;
      auto& mb = mblocks[iblk];
			...
			void (*f2) (int *,int *))
  {
   for(int ib=0;ib<nblocks;ib++)
    { 
     auto& mb = mblocks[ib];   // TODO:this may have to based on unique tag of p4est blocks这可能必须基于p4est块的唯一标记
