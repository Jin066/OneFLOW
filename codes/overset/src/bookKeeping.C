...
)
{
  for(int i=0; i<nsearch; i++) {
    ...
    sndPack[k].intData[ix++] = meshtag;           // Unique mesh tag独特的网格标签
    sndPack[k].intData[ix++] = isearch[3*i + 1];  // point ID  点id
    sndPack[k].intData[ix++] = i;                 // point ID on donor side 在贡献单元上，点的ID
    sndPack[k].intData[ix++] = isearch[3*i + 2];  // receptor block ID  接收者的ID
			      ...
			      int *nrecords)
{
  ...
  int verbose;
  //
  // first mark hole points 第一个标记孔点
  //
  ...
  // of hole points 将强制性条纹标记为孔点的邻居（高达nfringe深度）
  //
  ...
  if (receptorRes==BIGVALUE && resolutionScale==1.0)
    {
      clist=cancelList;
      //
      // go to the end of the list 到名单的末尾去
      //
...
void MeshBlock::reduce_fringes(void)
{
  ...
  // cell nodes are tagged  确保只标记部分iblank=1和iblank=-1的单元节点
  //
  //for(n=0;n<ntypes;n++)
  //  {
  //    nvert=nv[n];
