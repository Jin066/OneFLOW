...
void MeshBlock::search(void)
{
  ...
  int *dId;
  //
  // form the bounding box of the 形成查询点的边界框
  // query points
  //
  if (nsearch == 0) {
    donorCount=0;
...
findOBB(xsearch,obq->xc,obq->dxc,obq->vec,nsearch);
  ...
  for(n=0;n<ntypes;n++)
      ...
      for(i=0;i<nc[n];i++)
	{
	  ...
	  // overlap with the bounding box 找到与包围框重叠的每个单元格
	  //
	  ...
	  if (fabs(xd[0]) <= (dxc[0]+obq->dxc[0]) &&
	      ...
	      fabs(xd[2]) <= (dxc[2]+obq->dxc[2])) 
	    {
	      ...
	      // the QP bounding box  创建一个lifo堆栈，其中包含与QP包围框相交的所有单元格
	      //
	      icell[p]=iptr;
	      iptr=p;
	      cell_count++;
  ...
  // of each cell in the LIFO stack to build the  现在找到lifo堆栈中每个单元格的轴对齐包围框以构建ADT
  // ADT
  //

  if (elementBbox) TIOGA_FREE(elementBbox);
  if (elementList) TIOGA_FREE(elementList);
  elementBbox=(double *)malloc(sizeof(double)*cell_count*6); //单元边界箱
  elementList=(int *)malloc(sizeof(int)*cell_count); //单元列表
  //
  k=iptr;
  l=0;
  p=0;
  ...
  ndim=6; //?
  //
  adt->buildADT(ndim,cell_count,elementBbox); //构建ADT
  //
  ...
  //
  // create a unique hash 创建一个独特的Hash表
  //
  ...
  for(i=0;i<nsearch;i++)
	...
	adt->searchADT(this,dId,&(xsearch[3*i]));  //搜索ADT
        donorId[i]=dId[0];
      }
      else {
...
void MeshBlock::search_uniform_hex(void)
{
  ...
  double xvec[8][3];
  //
  // corners of a cube with of side 4*TOL 以原点为中心的边长为4*TOL的立方体的角
