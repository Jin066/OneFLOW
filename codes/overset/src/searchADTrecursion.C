...
void ADT::searchADT(MeshBlock *mb, int *cellIndex,double *xsearch)
{
  ...
  // ADT nodes  调用递归例程来检查具有ADT节点的交叉口
  //
  if (flag) searchIntersections(mb,cellIndex,adtIntegers,adtReals,
				coord,0,rootNode,xsearch,nelem,ndim); //从根节点开始搜索
}
			 ...
			 double *coord,int level,int node,double *xsearch,int nelem,int ndim)
{
  ...
  double *element; //改动了，原来 double element[ndim]
  element = (double *)malloc(sizeof(int) *ndim);
  bool flag;
  //
  for(i=0;i<ndim;i++)
  ...
  if (flag)
    {
      mb->checkContainment(cellIndex,adtIntegers[4*node],xsearch);//检查容量
      if (cellIndex[0] > -1 && cellIndex[1]==0) return;
    }
  //
  // check the left and right children  检查左右子节点
  // now
  //
  for(d=1;d<3;d++)
    {
