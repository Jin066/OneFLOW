...
void UINsVisterm::CmpFaceVisterm()
{
	...
	iinv.Fn[ug.fId] = (1 / 2) / (1 + gcom.xfn * l2rdx + gcom.yfn * l2rdy + gcom.zfn * l2rdz) * gcom.farea;   // μ / ( n * d ) 法向扩散项系数(改动)
	...
	iinv.ai1[ug.fId] += iinv.Fn[ug.fId];
	iinv.ai2[ug.fId] += iinv.Fn[ug.fId];
